...
////                        }
////                       THREAD SLEEP
                        log.info("Thread sleep starts....."+System.currentTimeMillis());
                        try{
                        Thread.sleep(waitingTime*60*1000);
                        log.info("Thread sleep finishes....."+System.currentTimeMillis());
                            metricScheduling = true;
                            rollback(deployedApps);
                            deployedApps = Collections.emptyList();
                            deployedSiddhiAppHoldersMap.remove(distributedSiddhiQuery.getAppName());
                            ServiceDataHolder.getResourcePool().getAppsWaitingForDeploy()
                                    .put(distributedSiddhiQuery.getAppName(), appsToDeploy);
                            log.info("Starting MetricBasedResourceAllocation...");
                            log.info("Waiting after un-deploying........");
                            long startWaitingTime = System.currentTimeMillis();
                            while(true){
                                if(System.currentTimeMillis() - startWaitingTime> 60*1000 ){
                                    break;
                                }
                            }
                            log.info("Starting after undeploying..........");
                            deploy(distributedSiddhiQuery , metricScheduling);
                        }
                        catch(InterruptedException e){
                            log.error("Interruption of the thread sleep",e);

                        }

                        //TIMER

//                        setupTimer(deployedApps,distributedSiddhiQuery,appsToDeploy,deployedSiddhiAppHoldersMap,waitingTime);
    ...
    public boolean unDeploy(String siddhiAppName) {
        log.info("undeploying by the manager.................... 2 ");
    ...
    private ResourceNode deploy(SiddhiQuery siddhiQuery, int retry, int parallelism , boolean metricScheduling) {
        ...
        if (siddhiQuery.isReceiverQuery()) {
            ...
            try{
                log.info("Autocommit enabled ? "+connection.getAutoCommit());
                log.info("Client information" +connection.getClientInfo());
                connection.setAutoCommit(true);
                statement = conn.createStatement();
            } catch (SQLException e){
                log.error("Error in creating the database connection",e);
            }
            if (metricScheduling) {
    ...
    private void setupTimer(List<SiddhiAppHolder> deployedApps,DistributedSiddhiQuery distributedSiddhiQuery,
                            List<SiddhiAppHolder> appsToDeploy, Map<String, List<SiddhiAppHolder>> deployedSiddhiAppHoldersMap, int waitingTime){
        ...
        schedule(new Callable() {
                                      public Object call() throws Exception {
                                          log.info("Executed!");
                                          reschedule(deployedApps,distributedSiddhiQuery,appsToDeploy
                                          ,deployedSiddhiAppHoldersMap,waitingTime);
                                          return "rescheduled!";
                                              }
                                          },
                        waitingTime,
    ...
    private void reschedule(List<SiddhiAppHolder> deployedApps,DistributedSiddhiQuery distributedSiddhiQuery,
                            List<SiddhiAppHolder> appsToDeploy, Map<String, List<SiddhiAppHolder>> deployedSiddhiAppHoldersMap, int waitingTime){
        metricScheduling = true;
        rollback(deployedApps);
        deployedApps = Collections.emptyList();

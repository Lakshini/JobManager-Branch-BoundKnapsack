...
public class MultipleKnapsack {
    ...
    public LinkedList<PartialSiddhiApp> executeBranchAndBoundKnapsack(LinkedList<PartialSiddhiApp> partialSiddhiApps) {
        ...
        for (Knapsack knapsack : knapsacks) {
            ...
            log.info("final tree node after traversing the tree === " + finalTreeNode);
            log.info("Sizeeeeeee" + finalTreeNode.getPartialSiddhiAppsOfNode().size());
            double totalCPU = 0.0;
            ...
            for (PartialSiddhiApp aa : knapsack.getpartialSiddhiApps()) {
                log.info(aa.getName() + "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
            }
            branchAndBoundTreeNodes.clear();
            feasibleTreeNodes.clear();
